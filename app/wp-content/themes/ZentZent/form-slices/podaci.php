...
<script>
	...
	var drugaadresa = $('#drugaadresa-form').clone(true);
	$('#drugaadresa-form').remove();
	...
	$document.on('change', '#drugaadresa', function() {
		...
		var drugaPolja = ['#drugaadresa-drzava', '#drugaadresa-ime', 'drugaadresa-prezime', '#drugaadresa-company', '#drugaadresa-adresa', 'drugaadresa-grad', 'drugaadresa-postanskibroj'];
		
		$.each(drugaPolja, function(key, value) {
			if (!($drugaadresaForm.find(value).attr('required'))) {
				$drugaadresaForm.find(value).attr('required', true);
			} else {
				$drugaadresaForm.find(value).removeAttr('required');
			}
		});
	});
	...
	// emaili moraju da budu isti
	$document.on('change', '#ponovi-email', function() {
		if ($(this).val() !== $('#email').val()) {
			$(this).css('background-color', '#c75b5b');
